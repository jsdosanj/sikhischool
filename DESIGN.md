...
| 2026-09-05 | Applying this system (Workstream A2-A5): `little-sparks` keeps its original Baloo 2 / Atkinson Hyperlegible typefaces, not this doc's Fraunces/Source Sans 3 | This doc's "don't look like cartoon-mascot competitors" rationale targets the 3-12 bands specifically. Atkinson Hyperlegible (built by the Braille Institute) was a deliberate pre-/early-reader legibility choice already in place, not incidental branding to override for consistency's sake. Color/spacing/radius scale still apply to `little-sparks` per the shared-token table above — only the two typefaces are exempted. |
| 2026-09-05 | `sikhi-school-studio` adopts this doc's dark-theme token values as its fixed identity, rather than building a `prefers-color-scheme` toggle across all three shells | No dark-mode toggle exists in the app yet. Studio was already the one shell with a fixed dark identity (pre-dates this doc), so it's the natural home for the real dark tokens rather than an approximation. `little-sparks`/`rising-school` ship the light tokens as their fixed identity. A genuine OS-preference-aware toggle across all three remains a follow-on (`TODOS.md`), not bundled into this pass. |
