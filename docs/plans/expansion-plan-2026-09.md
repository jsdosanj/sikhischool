...
- **A0. Design foundation: Apple's Human Interface Guidelines, principles not pixels (founder directive, 2026-09-04).** HIG's three governing principles — **Clarity** (legible type at every size, purposeful icons, content-first layout — no decoration competing with the thing the user is there to do), **Deference** (chrome recedes, content leads — matches the AI-slop blacklist's "cards earn their existence" rule already in this plan), **Depth** (layering/hierarchy communicates structure and order of operations, not flat sameness everywhere) — become the design system's north star, adapted for a K-12 web product rather than a native Apple app. Concretely, `/design-consultation` (A1) works from: a real type scale with generous line-height and Dynamic-Type-style scalable sizing (never below 16px body per §1.1's existing floor); a spacing/layout grid disciplined the way HIG's safe-area/margin system is; semantic color tokens (not just light/dark swaps but purpose-named — `color-primary-action`, `color-content-background` — the way HIG's system colors work); purposeful, restrained motion (state transitions that clarify what changed, never decorative animation, and honoring `prefers-reduced-motion`); and HIG's accessibility floor (44pt/px minimum touch targets — already this plan's stated floor — full VoiceOver/screen-reader equivalence, sufficient contrast in both themes). **Licensing constraint, binding (same rule class as §0's reference-repo licensing note): Apple's actual assets — SF Pro, SF Symbols — are licensed for apps built for Apple platforms only; a web product cannot use them directly without violating that license.** A1 selects an original, properly-licensed typeface and icon set that achieve the same clarity/legibility bar HIG describes, not a literal visual clone of Apple's look. The goal is a product that *feels* like it was designed with Apple-grade discipline and restraint — not one that borrows Apple's actual branding.
...
- Exit gate: 8+ step-types live, reused across at least 3 subjects each (proves genuine reuse, not per-subject one-offs — CLAUDE.md §2). **Partially cleared (2026-09-05): all 8 step-types from §3's taxonomy are built and registered** (multiple-choice, matching-pairs, speed-drill, translate, cloze, listening-comprehension, build-the-sentence, speaking-prompt). **The reuse-across-3-subjects half is not yet true** — that depends on actual lesson content authoring referencing them, which hasn't started (Phase 1 content authoring, not yet begun as of this note).
...
- **D1. Schema generalization. BUILT (2026-09-04), live in production.** `punjabiDictionary` → `dictionary {id, language, word, translation, partOfSpeech, synonyms[], exampleSentence, audioRef, gradeBandHint}` — `language: "punjabi"` migrates the existing rows untouched; `language: "english"` and the three new language slugs populate alongside.
...
- **D4. Click/lookup UX** (design-owned, A3): instant word lookup from any reading passage or Santhya Path text, inspired by LLPlayer/Dictionariez's double-click pattern — reimplemented natively, not the extension's code. **Two intentional layouts, not one squeezed into both (design review finding, §15):** desktop gets a lightweight inline popover anchored to the word; mobile (<768px, no hover) gets a full-width bottom sheet (slide up, dismiss via swipe-down or tap-outside) — an inline popover next to a tapped word clips or crowds on a small screen, the exact "stacked on mobile, not intentional" failure mode. **BUILT (2026-09-05), live in production** — a `[[word]]` content-markup convention (documented in `docs/CONTENT-AUTHORING.md`) plus a `WordLookup` component shipping both required layouts, wired into the lesson page for `ela`/`punjabi`/`sikhi` subjects today (the only ones currently in the dictionary-language map). **Not yet wired into Santhya Path** despite this bullet's own scope — that integration remains open work, not silently dropped.
- Exit gate: every published language has ≥1 populated dictionary; English spelling-bee lists cover every grade band; lookup UX ships on at least the ELA reading surface and Santhya Path. **Not yet cleared:** D2 (English dictionary/thesaurus/spelling-bee) and D3 (populated per-language dictionaries beyond Punjabi's pre-existing data) haven't been built — the `dictionary` table has real Punjabi rows only as of this note, nothing for English/Spanish/Mandarin/French yet. D4's lookup UX is live but only on the lesson-page surface, not Santhya Path.
